...
class Gazebo4WSEnvLvl1(gym.Env):
    def __init__(self):
        super(Gazebo4WSEnvLvl1, self).__init__()
        self.node = rclpy.create_node('rl_env_lvl1')
        ...
        print("[INFO] Hệ thống sẵn sàng. Bắt đầu bài học Đỗ Xe Chính Xác!")
    ...
    def odom_callback(self, msg):
        ...
        if not getattr(self, 'got_pose', False):
            self.got_pose = True
    ...
    def step(self, action):
        ...
        # --- LUẬT MỚI 1: Cấp quyền đạp phanh ---
        # Ánh xạ action từ [-1, 1] sang [-5.0, 20.0] rad/s
        # (Nếu AI chọn action âm, xe sẽ phanh lại hoặc lùi nhẹ)
        ...
        if self.current_step > 10:
            if min_lidar_dist < 0.26:
                reward -= 100.0 
                print(f"[Va chạm] Đâm tường ở {min_lidar_dist:.2f}m. TRỪ 100đ!")
                done = True

        # --- LUẬT MỚI 2: Đỗ xe chính xác (Precision Parking) ---
        # Chỉ xét thắng nếu lọt vào trong hình tròn vàng (bán kính 0.4)
        if current_dist < 0.4:
            # KIỂM TRA PHANH: Phải đi cực chậm (< 2.0 rad/s) mới được tính là đỗ xe
            if abs(forward_speed) < 2.0:
                reward += 300.0 
                print(">>> XUẤT SẮC: ĐÃ ĐỖ XE AN TOÀN TẠI TÂM ĐÍCH! <<<")
                done = True
            else:
                # Nếu lọt tâm mà phóng như bay, thưởng chút xíu khuyến khích ở lại
                # nhưng ép nó tự hiểu ra phải hãm phanh lại nếu muốn thắng lớn.
                reward += 0.5 

        # Cảnh báo trượt quá đà
        if self.robot_pos[0] > 5.3:
             reward -= 100.0
